...
class _ValidateReturnsState extends State<ValidateReturns> {
  ...
  Future<void> removeBookRequest() async {
    final userDoc = _firestore.collection('users').doc(email);
    final bookQuery = await _firestore.collection('books').where('title', isEqualTo: title).get();
    setState(() {
      spin = true;
    ...
    try {
      ...
      if (userSnapshot.exists) {
        ...
        if (bookQuery.docs.isNotEmpty) {
          ...
          if (existingBooks.containsKey(title)) {
            // Remove the book from the user's collection
            existingBooks.remove(title);
      ...
      setState(() {
        spin = false; // Reset sendingEmail to false when finished removing book
      });
    }
  }
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [
                  Colors.red, // Dark purple color
                  Colors.deepOrange, // White color
                ],
              ),
            ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(
                          child: TextUtil(
                            text: "Return Book", weight: true, size: 30.sp,
                          ),
                        ),
                        Spacer(),
